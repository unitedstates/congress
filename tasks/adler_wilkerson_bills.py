...
import utils
from tasks import Task, merge


class AdlerWilkersonBills(Task):

    def __init__(self, options=None, config=None):
        super(AdlerWilkersonBills, self).__init__(options, config)

    def run(self):
        # Download the TSV file.
        cache_zip_path = "adler-wilkerson-bills.zip"
        url = "http://congressionalbills.org/billfiles/bills80-92.zip"
        self.download(url, cache_zip_path, merge(self.options, {'binary': True, 'needs_content': False}))

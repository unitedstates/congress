...
import utils
from votes import Votes, format_datetime
from tasks import current_congress


class Voteview(Votes):

    def __init__(self, options, config):
        super(Voteview, self).__init__(options, config)
        # load some hard-coded codes
        self.special_vote_options = {}
        for rec in csv.reader(self.storage.fs.open("tasks/voteview_codedoptions.csv")):
            if rec[0] == "vote date":
                continue    # header
            self.special_vote_options[rec[1]] = (rec[2], dict((int(r.split(':', 1)[0]), r.split(':', 1)[1]) for r in rec[3].split(';')))
    ...
    def run(self):
        ...
        session_dates = list(csv.DictReader(StringIO.StringIO(self.download("http://www.govtrack.us/data/us/sessions.tsv").encode("utf8")), delimiter="\t"))

        # download the vote data now
        if chamber and chamber in ["h", "s"]:
            votes = self.get_votes(chamber, congress, session_dates)
        else:
            votes = self.get_votes("h", congress, session_dates) + self.get_votes("s", congress, session_dates)

        self.process_set(votes, self.put_vote)

    def get_votes(self, chamber, congress, session_dates):
        logging.warn("Getting votes for %d-%s..." % (congress, chamber))

        vote_list_url, rollcall_list_url = self.vote_list_source_urls_for(congress, chamber)

        # Load the ORD file which contains the matrix of how people voted.

        vote_list_file = self.download(vote_list_url, self.cache_file_for(congress, chamber, "ord"), self.options).encode("utf-8")
        if not vote_list_file:
            logging.error("Couldn't download vote list file.")
            return None

        vote_list = self.parse_vote_list_file(vote_list_file)
        votes, presidents_positions = self.build_votes(vote_list)

        # Load the DTL file which lists each roll call vote with textual metadata.

        rollcall_list_file = self.download(rollcall_list_url, self.cache_file_for(congress, chamber, "dtl"), self.options).encode("utf-8")
        if not rollcall_list_file:
            logging.error("Couldn't download rollcall list file.")
            return None
        rollcall_list = self.parse_rollcall_dtl_list_file(rollcall_list_file, congress)
        ...
        for rollcall_number in rollcall_list:
            ...
            if rollcall["date"]:
                d_congress, d_session = self.session_from_date(rollcall["date"], session_dates)
        ...
        for rollcall_number in rollcall_list:
            ...
            s_congress, session = self.session_from_date(rollcall["date"], session_dates)
            ...
            if self.options.get("session") and session != self.options["session"]:
                continue

            rollcall['result'] = "unknown"
            if "description" in rollcall:
                self.parse_rollcall_description(rollcall)

            # Make the votes dictionary, but also replace the description
            # text when it contains coded vote information.
            votes_dict = self.build_votes_dict(vote_results, rollcall)
            ...
            vote_output = {
                ...
                "type": self.normalize_vote_type(rollcall["description"]) if "description" in rollcall else None,
    ...
    def vote_list_source_urls_for(self, congress, chamber):
        url = "http://www.voteview.com/%s%02d.htm" % (("house" if chamber == "h" else "senate"), congress)
        index_page = self.download(url, self.cache_file_for(congress, chamber, "html"), self.options)
    ...
    def parse_voteview_vote_code(vote_code):
        ...
        in the House on the passage of bills. A paired vote is when two members
        plan to be absent in a way that does not affect the vote outcome.
        You can see in the Congressional record who is paired with who.
        Sometimes the pairings are for a particular vote, other pairings
        are "until further notice." The paired members are recorded as not
        voting. A live pair is when one half of the pair is present and
        withdraws their vote and votes present because the other half of the
        pair isn't there. Live pairs aren't recorded in this data and are
        treated simply as pairs (and thus for us, not voting). Some paired
        members are recorded simply as present in this data --- not clear why that would be.
        ...
        @param vote_code: integer to map to a tuple of strings
        @type vote_code: int
        @return: tuple of standard vote options, voteview string
        @rtype: tuple
        """
    ...
    def parse_vote_list_file(self, vote_list_file):
        ...
        for vote_list_line in vote_list_file.split("\r\n"):
            ...
            vote_info = self.extract_vote_info_from_parsed_vote_list_line(self.parse_vote_list_line(vote_list_line))

            vote_info["state"] = self.get_state_from_icpsr_state_code(vote_info["icpsr_state"]) if vote_info["icpsr_state"] is not None else None
            vote_info["party"] = self.get_party_from_icpsr_party_code(vote_info["icpsr_party"]) if vote_info["icpsr_party"] is not None else None
    ...
    def parse_rollcall_dtl_list_file(self, rollcall_dtl_list_file, congress):
        ...
        for rollcall_dtl_list_line in rollcall_dtl_list_file.split("\r\n"):
            ...
            rollcall_dtl_list_line_info = self.extract_rollcall_info_from_parsed_rollcall_dtl_list_line(self.parse_rollcall_dtl_list_line(rollcall_dtl_list_line))

            if rollcall_dtl_list_line_info["line"] == 1:
                rollcall_info = {}

                rollcall_dtl_list_first_line_parts = self.parse_rollcall_dtl_list_first_line(rollcall_dtl_list_line_info["text"])
                ...
                rollcall_info["date"] = self.parse_rollcall_dtl_date(rollcall_info["date_unparsed"])
    ...
    def build_votes_dict(self, votes_list, rollcall):
        if rollcall.get("description") in self.special_vote_options:
            ...
            new_description, vote_codes = self.special_vote_options[original_description]
            ...
            for v in votes_list:
                v["vote"], v["voteview_votecode_extra"] = self.parse_voteview_vote_code(v["vote"])
    ...
    def put_vote(self, vote):
        self.output_vote(vote, id_type="bioguide")

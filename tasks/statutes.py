...
class Statutes(Task):
    ...
    def __init__(self, options=None, config=None):
        super(Statutes, self).__init__(options, config)

    def run(self):
        root_dir = self.storage.data_dir + '/fdsys/STATUTE'

        # TODO: transition to self.storage object
        if "volume" in self.options:
            to_fetch = glob.glob(root_dir + "/*/STATUTE-" + str(int(self.options["volume"])))
        elif "volumes" in self.options:
            start, end = self.options["volumes"].split("-")
            to_fetch = []
            for v in xrange(int(start), int(end) + 1):
                to_fetch.extend(glob.glob(root_dir + "/*/STATUTE-" + str(v)))
        elif "year" in self.options:
            to_fetch = glob.glob(root_dir + "/" + str(int(self.options["year"])) + "/STATUTE-*")
        elif "years" in self.options:
            start, end = self.options["years"].split("-")
            to_fetch = []
        ...
        self.process_set(to_fetch, self.proc_statute_volume)

    def proc_statute_volume(self, path):
        ...
        self.fetch_committee_names(congress)
        ...
        for bill in mods.findall("/mods:relatedItem", mods_ns):
            ...
            subject = getattr(descriptor, 'text', None)
            ...
            if cong_committee is not None:
                ...
                committee_info = {
                    "committee": committee,
                    "activity": [],  # XXX
                    "committee_id": self.COMMITTEE_NAMES[committee] if committee in self.COMMITTEE_NAMES else None,
                }
            ...
            if not self.options.get('textversions', False):
                Bills(self.options, self.config).output_bill(bill_data)
            ...
            if os.path.exists(pdf_file):
                ...
                if self.options.get("linkpdf", False):
                    os.link(pdf_file, dst_path + "/document.pdf")  # a good idea
                if self.options.get("extracttext", False):
    ...
    def output_for_bill_version(self, bill_version_id):
        bill_type, number, congress, version_code = Bills.split_bill_version_id(bill_version_id)
        return "%s/%s/bills/%s/%s%s/text-versions/%s/data.json" % (self.storage.data_dir, congress, bill_type, bill_type, number, version_code)
